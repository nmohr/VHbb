...
systhistosarray=[]
Coco=0 #iterates over systematics * (up,down)
...
for sys in systematics:
    ...
    for Q in range(0,2):
        ...
        for i in range(0,len(systhistosarray[Coco])):
            systhistosarray[Coco][i].SetName('%s%s%s'%(discr_names[i],systematicsnaming[sys],UD[Q]))
            ...
            histPdf = ROOT.RooDataHist('%s%s%s'%(discr_names[i],systematicsnaming[sys],UD[Q]),'%s%s%s'%(discr_names[i],systematicsnaming[sys],UD[Q]),obs,systhistosarray[Coco][i])
            getattr(WS,'import')(histPdf)
